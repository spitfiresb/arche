...
The combined `reformatting` branch currently displays the static
`lookout-panorama-v2.png` on Home. This directory preserves the editable
layer composition from the former illustration branch for further artwork work.
The inspector composes 23 static artwork layers. The mountain, three depths of clouds,
...
The former stars and interactive moon/sun remain in `home-scene.js`, which is
not loaded by the current homepage. About remains the reformatted timeline.

The former homepage fitted these layers into a shorter band and filled the extra
width with generated side terrain. The current homepage uses that panorama directly.
See [panorama.md](panorama.md) for the side asset, alignment and generation prompt.

## Preview and edit

Run `python3 tools/serve.py`, then visit http://localhost:8712/__scene.
...
`public/assets/img/home/layers/scene.json` with that file to save the composition
for the inspector. This does not update the static panorama used by Home.
No inspector controls or scene animation run on the actual Home page.
...
All coordinates use a 1536 × 1024 artboard. The inspector's Home crop displays
`0 430 1536 594`. Manifest order is back to front. Source layers are
