...
//
// Counts distinct daily visitors over the last 30 days. Every page calls this
// on load; the home page refreshes its visible status every 30 seconds.
//
// One request does the whole job — record, sweep, and read the visitor count — in a
...
export async function onRequestPost({ request, env, waitUntil }) {
  try {
    ...
    try {
      ...
      /* An empty request reads status without registering a visit. */
    }
    ...
    return json({ visits, place, track }, 200);
  } catch (error) {
...
export async function refreshSpotify(db, env, row) {
  try {
    ...
    // Never let the music corner take the visitor count down with it.
