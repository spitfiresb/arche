(function () {
  ...
  function buildFloor() {
    ...
    floorSvg.style.left = f1(-mainL) + 'px';
    floorSvg.style.top = f1(fy - gy) + 'px';
    // the meadow itself is drawn by camp.js, shared with the home page
    const camp = drawCamp(floorSvg, {
      fw, gy, cliffX, cliff: true, stick: !stickTaken });
    floorFlame = camp.flame;
    floorStick = camp.stick;
    // where the campfire and the stump sit in the rig's rope-relative
    // space, so he knows where to walk once he's off the rope
    campX = camp.fireX;
    stumpX = camp.stumpX;
  }
